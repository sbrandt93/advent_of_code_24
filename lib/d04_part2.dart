...
int findMatches(String searchedWord, List<String> riddle) {
  int count = 0;
  Map<Map<int, int>, int> indicesOfA = {};
  Map<Map<int, int>, int> indicesOfAComplete = {};
        ...
        for (final direction in directions.keys) {
          ...
          if (nextX >= 0 && nextX < riddle.length && nextY >= 0 && nextY < riddle[nextX].length) {
            if (searchNextLettersInDirection(nextX, nextY, direction, searchedWord, 1, riddle)) {
              ...
              Map<int, int>? found;
              final keys = indicesOfA.keys;
              for (int k = 0; k < indicesOfA.length; k++) {
                final key = keys.elementAt(k);
                if (compareMaps(key, newCoordinate)) {
                  found = key;
                  break;
                }
              }
              if (found != null) {
                indicesOfAComplete[found] = 2;
                indicesOfA.remove(found);
              } else {
  ...
  //   }
  // }
  print('countX ${indicesOfAComplete.length}');
  return count;
}
